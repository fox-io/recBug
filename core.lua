...
output_frame:SetFontObject(SystemFont_Med1)
...
local function show_error(index)
	if errors[index] then
		-- Generate the error text and place it in the error frame.
		...
		-- Set the next button to the next highest index (even if it does not exist).
		next_button.id = index + 1
		-- Disable the next button if the button's error index is not assigned.
		if not errors[next_button.id] then
			next_button:Disable()
			next_button:SetAlpha(.25)
		-- Enable the next button if the button's error index is assigned.
		else
			next_button:Enable()
			next_button:SetAlpha(1)
		end
		
		-- Set the previous button to the next lowest index (even if it does not exist).
		previous_button.id = index - 1
		-- Disable the previous button if the button's error index is not assigned.
		if not errors[previous_button.id] then
			previous_button:Disable()
			previous_button:SetAlpha(.25)
		-- Enable the previous button if the button's error index is assigned.
		else
			previous_button:Enable()
			previous_button:SetAlpha(1)
		end
		
		-- Ensure the scroll frame is the proper size based on the content.
		error_scroll:UpdateScrollChildRect()
		
		-- Prevent the frame from auto trapping focus.
		output_frame:ClearFocus()
		
		-- Display the error frame if it is not already shown, and the user has not manually closed it before this error occurred.
		if (not error_frame:IsShown()) and (not error_frame.closed) then
			error_frame:Show()
		-- Display a chat message to inform the user if the frame has been manually closed, and if a notification has not been sent since the last close.
